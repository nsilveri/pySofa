...
if data:
    ...
    print(f"Chiavi se dict: {data.keys() if isinstance(data, dict) else 'Non è dict'}")
    # Gestisci sia se data è una lista diretta che se è un dict con 'events'
    if isinstance(data, dict):
        if 'events' in data:
            events = data['events']
        elif 'error' in data:
            print(f"Errore dall'API: {data['error']}")
            events = []
        else:
    ...
    if events:
        db_module.save_matches_to_db(events)    
    ...
    for i, event in enumerate(events):
        ...
        # Controlla se il match è già stato processato
        conn = db_module.create_connection()
        if conn and db_module.match_exists(conn, match_id):
            print(f"Match {match_id} già processato, skip.")
            conn.close()
            continue
        conn.close()
