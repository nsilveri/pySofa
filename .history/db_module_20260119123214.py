...
def create_graphics_table(conn):
    create_table_query = """
    DROP TABLE IF EXISTS match_graphics;
    CREATE TABLE match_graphics (
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(create_table_query)
        conn.commit()
        print("Tabella 'match_graphics' creata.")
    except Exception as e:
...
def insert_graphics(conn, match_id, graphics):
    ...
    if 'home' in graphics:
        for event in graphics['home']:
            event_type = event.get('type', '').lower()
            if 'goal' in event_type:
                counts['goals_home'] += 1
            elif 'yellow_card' in event_type or 'yellow' in event_type:
                counts['yellow_cards_home'] += 1
            elif 'red_card' in event_type or 'red' in event_type:
                counts['red_cards_home'] += 1
            elif 'substitution' in event_type or 'sub' in event_type:
                counts['substitutions_home'] += 1
            elif 'foul' in event_type:
                counts['fouls_home'] += 1
            elif 'corner' in event_type:
                counts['corners_home'] += 1
            elif 'offside' in event_type:
                counts['offsides_home'] += 1
    
    if 'away' in graphics:
        for event in graphics['away']:
            event_type = event.get('type', '').lower()
            if 'goal' in event_type:
                counts['goals_away'] += 1
            elif 'yellow_card' in event_type or 'yellow' in event_type:
                counts['yellow_cards_away'] += 1
            elif 'red_card' in event_type or 'red' in event_type:
                counts['red_cards_away'] += 1
            elif 'substitution' in event_type or 'sub' in event_type:
                counts['substitutions_away'] += 1
            elif 'foul' in event_type:
                counts['fouls_away'] += 1
            elif 'corner' in event_type:
                counts['corners_away'] += 1
            elif 'offside' in event_type:
                counts['offsides_away'] += 1
    
    insert_query = """
    INSERT INTO match_graphics (
        match_id, goals_home, goals_away, yellow_cards_home, yellow_cards_away,
        red_cards_home, red_cards_away, substitutions_home, substitutions_away,
        fouls_home, fouls_away, corners_home, corners_away, offsides_home, offsides_away
    ...
    values = (
        ...
        counts['goals_home'],
        counts['goals_away'],
        counts['yellow_cards_home'],
        counts['yellow_cards_away'],
        counts['red_cards_home'],
        counts['red_cards_away'],
        counts['substitutions_home'],
        counts['substitutions_away'],
        counts['fouls_home'],
        counts['fouls_away'],
        counts['corners_home'],
        counts['corners_away'],
        counts['offsides_home'],
        counts['offsides_away']
    )
    try:
        with conn.cursor() as cursor:
...
def create_statistics_table(conn):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(create_table_query)
        conn.commit()
        print("Tabella 'match_statistics' creata.")
    except Exception as e:
