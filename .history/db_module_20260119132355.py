...
def insert_statistics(conn, match_id, statistics):
    ...
    # Extract values for columns
    stats_dict = {}
    stats_list = statistics.get('statistics', statistics) if isinstance(statistics, dict) else statistics
    if isinstance(stats_list, list):
        for group in stats_list:
            if 'statisticsItems' in group:
                for stat in group['statisticsItems']:
                    name = stat.get('name', '').lower().replace(' ', '_').replace('%', '').replace('(', '').replace(')', '').replace('/', '_')
                    home_val = stat.get('home', '0')
                    away_val = stat.get('away', '0')
                    print(f"Processing stat: {name}, home: {home_val}, away: {away_val}")  # Debug
                    
                    # Converti in tipi appropriati
                    if 'ball_possession' in name or 'possession' in name:
                        # For possession, it's like '34%', so strip %
                    ...
                    elif 'shots_on_target' in name or 'shots_on_goal' in name:
                        stats_dict['shots_on_target_home'] = int(home_val)
                        stats_dict['shots_on_target_away'] = int(away_val)
                    elif 'total_shots' in name:
                        stats_dict['shots_total_home'] = int(home_val)
                        stats_dict['shots_total_away'] = int(away_val)
                    elif 'passes' in name and 'accurate' not in name:
                        stats_dict['passes_total_home'] = int(home_val)
                        stats_dict['passes_total_away'] = int(away_val)
                    elif 'pass_accuracy' in name or ('accurate_passes' in name and '%' in home_val):
                        # For accurate passes, it's like '141/181 (78%)', but in the data it's '141' for home, but wait, in the print it's '141' for accurate passes.
                        # Actually, for 'Accurate passes', home: '141', away: '470'
                        # But to get accuracy, perhaps calculate if needed, but for now, since it's total passes, wait.
                        # The name is 'Accurate passes', so perhaps map to passes_total, but accuracy is separate.
                        # In the data, there's 'Passes' and 'Accurate passes'.
                        # For 'Passes': home '181', away '511'
                        # For 'Accurate passes': '141', '470'
                        # So, passes_total is 'Passes', pass_accuracy is accurate/total *100
                        if 'accurate_passes' in name:
                            # But home_val is '141', but to get accuracy, need total.
                            # Perhaps skip for now, or add later.
                            pass
                        else:
    ...
    print(f"Stats dict extracted: {stats_dict}")  # Debug
