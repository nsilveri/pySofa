...
def create_connection(dbname='football_db', user='postgres', password='your_password', host='localhost', port='5432'):
    try:
...
def create_statistics_table(conn):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(create_column_query)
        conn.commit()
        print("Tabelle 'match_statistics_json' e 'match_statistics_column' create o ricreate.")
    except Exception as e:
...
def insert_statistics(conn, match_id, statistics):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # Read back the JSON from the table
            select_json_query = "SELECT statistics FROM match_statistics_json WHERE match_id = %s;"
            cursor.execute(select_json_query, (match_id,))
            row = cursor.fetchone()
            if row:
                stored_statistics = row[0]  # This is the JSONB object
                # Now insert into columns using the stored JSON
                insert_column_query = """
