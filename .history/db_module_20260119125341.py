...
def insert_graphics(conn, match_id, graphics):
    ...
    """ + ", ".join([f"momentum_{i} = EXCLUDED.momentum_{i}" for i in range(1, 91)])
...
def insert_statistics(conn, match_id, statistics):
    ...
    if 'statistics' in statistics:
        for stat in statistics['statistics']:
            ...
            if 'momentum' in name:
                stats_dict['momentum_home'] = float(home_val.rstrip('%')) / 100 if '%' in home_val else float(home_val) / 100
                stats_dict['momentum_away'] = float(away_val.rstrip('%')) / 100 if '%' in away_val else float(away_val) / 100
