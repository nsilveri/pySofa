...
def create_graphics_table(conn):
    # Crea colonne per ogni minuto da 1 a 90
...
def insert_graphics(conn, match_id, graphics):
    ...
    insert_query = f"""
    INSERT INTO match_graphics (match_id, {columns})
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(insert_query, [match_id] + values)
        conn.commit()
