...
if data:
    ...
    # Ottieni e salva i grafici per ciascuna partita (limitato a 5 per test)
    for event in data['events'][:5]:
        match_id = event['id']
        graphics = get_matches_per_day.get_graphics_per_match(match_id)
        if graphics:
            db_module.save_graphics_to_db(match_id, graphics)
        else:
            print(f"Nessun grafico trovato per match {match_id}")else:
